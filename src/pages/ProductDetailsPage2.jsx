...
// Import static fallbacks (yeh file already exist karti hai)
import {
...
const ProductDetailsPage = () => {
    ...
    const [selectedRatti, setSelectedRatti] = useState(null);
    ...
    const displayPrice = selectedRattiObj ? parseFloat(selectedRattiObj.price) : afterPrice;

    // Discount
    const discountPercent = beforePrice > displayPrice && displayPrice > 0
        ? Math.round(((beforePrice - displayPrice) / beforePrice) * 100)
        : 0;
    ...
    const youMayAlsoLike = fallbackYouMayAlsoLike;
    ...
    const handleAddToCart = () => {
        dispatch(addToCart({
            ...
            selectedRatti,
            price: displayPrice,
            selectedRattiPrice: selectedRattiObj?.price
        }));
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-start">
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="flex items-baseline gap-3 flex-wrap">
                            ...
                            {beforePrice > displayPrice && (
                                <span className="text-xl text-gray-400 line-through">₹{beforePrice.toLocaleString()}</span>
                            )}
                            {discountText && (
                                <span className="bg-green-100 text-green-700 px-3 py-1 rounded-full text-sm font-bold">{discountText}</span>
                            )}
                        </div>
                        ...
                        {rattiOptions.length > 0 && (
                            <div>
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {rattiOptions.map((opt) => (
                                        <button
                                            ...
                                            className={`px-4 py-2 border rounded-lg text-sm font-medium transition-all cursor-pointer ${selectedRatti === opt.ratti.toString() ? "border-amber-600 bg-amber-50 text-amber-700" : "border-gray-300 hover:border-gray-400 text-gray-700"}`}
                                        >
                                            {opt.ratti}
                                        </button>
                                    ))}
                                ...
                                {selectedRattiObj && (
                                    <p className="text-xs text-gray-500 mt-2">Price: ₹{selectedRattiObj.price} for {selectedRatti} Ratti</p>
                                )}
                            </div>
                        )}
